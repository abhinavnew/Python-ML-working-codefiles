f# -*- coding: utf-8 -*-
"""
Created on Wed May 26 17:37:30 2021
...
####NO OUTLIER TREATMENT produces best results:tried ul,ll 1,99 and 5,95##############
...
#     emp[j]=treat_outlier_5_95_winsor(emp[j])  
...
emp.to_csv(r'E:\AbhinavB\Kaggle\Python related\DSBA related\Competions\practice hackathon\emp_onehot.csv')
...
xgbobj=XGBClassifier()
xgb_mod=xgbobj.fit(trainset,train_labels)



##make predictions

pred_train=xgb_mod.predict(trainset)
...
pred=xgb_mod.predict(testset)
print(pred.size)
print("Accuracy on VALIDATE set= ",accuracy_score(pred,test_labels))

# ##Save the trained model on disk 
# pickle.dump(xgb_mod,open("xgbmod1_withpickle.dat","wb"))
...
#plot_importance(loaded_xgb_mod_disk)

# xgb_mod2=xgbobj.fit(all_ind_ds,all_labels)
# pred2=xgb_mod2.predict(testset)
# print(pred2.size)
...
probs_xgb = xgb_mod.predict_proba(trainset)
# keep probabilities for the positive outcome only
probs_xgb = probs_xgb[:, 1]
# calculate AUC
auc_xgb = roc_auc_score(train_labels, probs_xgb)
print('AUC: %.3f' % auc_xgb)
##Calc ROC curve and threshold 
fpr4, tpr4, thresholds_xgb = roc_curve(train_labels, probs_xgb)
...
for j in np.arange(0.1,1,0.1):
    ...
    print(round(custom_prob,3),round(metrics.accuracy_score(train_labels,custom_cutoff_data),4))
##0.6-0.9986 highest
...
##Testset UNKNOWN DATA  processing 
print(unktestset_orig.shape)
id=unktestset['EmployeeID']
print(test_unknown.shape)
pred_unk=xgb_mod.predict(test_unknown)
pred_unk=pd.Series(pred_unk.astype(int),name="Attrition")
type(pred_unk)
len(pred_unk)
# snapshotdate = datetime.datetime.today().strftime('%d-%m-%Y_%H:%M:%S')
# print(snapshotdate)
...
result_xgb.to_csv(r'E:\AbhinavB\Kaggle\Python related\DSBA related\Competions\practice hackathon\submission files\result_xgb_18.csv',index=False)
