# -*- coding: utf-8 -*-
"""
Created on Mon May 24 12:21:41 2021
...
nn_mod1 = GridSearchCV(estimator = nn_cl, param_grid = param_grid3, cv = 10,verbose=20)
...
probs4 = nn_mod1_bst.predict_proba(trainsetnn)
...
probs3 = nn_mod1_bst.predict_proba(testsetnn)
# keep probabilities for the positive outcome only
probs3 = probs3[:, 1]
# calculate AUC
...
nn_mod2 = GridSearchCV(estimator = nn_cl, param_grid = param_grid_full, cv = 10,verbose=20)

nn_mod2.fit(all_ind_ds_nn,all_labels_nn)

print("The best parameters found via gridsearch for NN Model are =",nn_mod2.best_params_)
nn_mod2_bst=nn_mod2.best_estimator_
...
pred_unk_nn=nn_mod1_bst.predict(test_unknown_nn)
pred_unk_nn=pd.Series(pred_unk_nn.astype(int),name="Attrition")

pred_unk_full=nn_mod2_bst.predict(test_unknown_nn)
pred_unk_full=pd.Series(pred_unk_nn.astype(int),name="Attrition")
...
result_nn=pd.concat([id,pred_unk_full],axis=1)
...
result_nn.to_csv(r'E:\AbhinavB\Kaggle\Python related\DSBA related\Competions\practice hackathon\submission files\result_nn_7_outlier199_full.csv',index=False)
