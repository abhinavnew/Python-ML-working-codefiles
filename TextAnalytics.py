# -*- coding: utf-8 -*-
"""
Created on Sun May 30 18:23:59 2021
...
x = 'Product received is genuine and worth purchasing. Product in hand is exactly same as that mentioned here on Amazon. Masks are comfortable to wear and yes has 3 protective layers. Earlier I purchased only 10 masks to confirm whether the product is matching the requirements, after which I ordered the set of more 100 masks. Satisfied with the product quality and delivery at the same time. Thanks'

x1='I have participated in the Great Learning Hackathon and got 3rd rank in the Hackathon.'

b=x1.split()

a=x.split()

df=pd.DataFrame({'text': 'I have participated in the Great Learning Hackathon and got 3rd rank in the Hackathon'},index=[0])
df['word_count']=df['text'].apply(lambda x:len(str(x).split()))
all_words_freq = nltk.FreqDist(df['text'])


a[0]


y=pd.DataFrame({'Review_text': 'Product received is genuine and worth purchasing. Product in hand is exactly same as that mentioned here on Amazon. Masks are comfortable to wear and yes has 3 protective layers. Earlier I purchased only 10 masks to confirm whether the product is matching the requirements, after which I ordered the set of more 100 masks. Satisfied with the product quality and delivery at the same time. Thanks'},index=[0])

y['Review_text']

y['word_count']=y['Review_text'].apply(lambda x:len(str(x).split()))

sw=stopwords.words('english')

y['count_stop'] = y['Review_text'].apply(lambda x: len([x for x in x.split() if x in sw]))

st=PorterStemmer()


##converting to lower case 
y['all_words_small']=y['Review_text'].apply(lambda x: " ".join(x.lower() for x in x.split()))

##removing all stop words

y['processed_text']=y['all_words_small'].apply(lambda x: " ".join([st.stem(x) for x in x.split()])

                                               
# Only keeping the words which are not the 'stopwords')
all_words_clean=[word for word in y['processed_text'] if word not in sw]
