# -*- coding: utf-8 -*-
"""
Created on Sat Nov 20 15:25:15 2021
...
# trainset,train_labels=oversample.fit_resample(trainset,train_labels)


        
####XGBoost without any SMOTE 
##Xtreme Gradient Boosting -On SMOTE'd trainset
...
print(end_time2)
train_time=timedelta(seconds=end_time2 - start_time2)
print("training time for xgb model is ",train_time)
...
print("Recall score of Tuned xgb model with DEFAULT THRESHOLD on TEST set=",recall_score_test_bst_xgb)
#auc
...
for j in np.arange(0.1,1,0.1):
    ...
    for i in range(0,len(train_labels)):  #defining a loop for the length of the train data
        if np.array(probs_train_xgb)[i] > custom_prob:#issuing a condition for our probability values to be 
            #greater than the custom cutoff value
            a=1 #if the probability values are greater than the custom cutoff then the value should be 1
        else:
            a=0 #if the probability values are less than the custom cutoff then the value should be 0
        custom_cutoff_data.append(a) #adding either 1 or 0 based on the condition to the end of the list defined by us
    print(round(custom_prob,3),metrics.recall_score(train_labels,custom_cutoff_data))

# probs_test_xgb = probs_test_xgb[:, 1]
data_pred_custom_cutoff=[]
for i in range(0,len(probs_test_xgb)):
    if np.array(probs_test_xgb)[i]>0.1:
        a=1
    else:
        a=0
    data_pred_custom_cutoff.append(a)

recall_score_opt=recall_score(test_labels,data_pred_custom_cutoff)
print("Recall with opt threshold",recall_score_opt)
...
plt.plot([0, 1], [0, 1], linestyle='--')
# plot the roc curve for the model
plt.plot(fpr3_test, tpr3_test, marker='.')
# show the plot
plt.show()
print("Rounder value =",round(auc_nn_test)*100)

####result collation 
index=['Train Accuracy','Train AUC', 'Test AUC','Test Accuracy','Test Recall']
data = pd.DataFrame({
      
        'XGBoost':[acc_score_train_bst_xgb,   auc_train_xgb,  auc_test_xgb,   acc_score_test_bst_xgb,    recall_score_test_bst_xgb],
        'Neural Net':[nn_acc_train,   auc_nn_train,   auc_nn_test,    nn_acc_test,    nn_recall_test]
