# -*- coding: utf-8 -*-
"""
Created on Sun May  2 19:34:35 2021
...
trucks_orig=pd.read_csv(r'E:\AbhinavB\Kaggle\Python related\DSBA related\Week16-LDA\trucks.csv')
trucks=trucks_orig.copy()
trucks.drop(trucks.columns[trucks.columns.str.contains('unnamed',case = False)],axis = 1, inplace = True)


trucks.info()

##Univariate analysis 
print(trucks['Freq'].unique())
print(trucks['period'].unique())
print(trucks['parked'].unique())
print(trucks['collision'].unique())


##duplicates
dups=trucks.duplicated()
print("The num of duplicate records are =",dups.sum())
trucks[dups]
trucks.drop_duplicates(inplace=True)
print(trucks.shape)
...
ll1,ul1=getupperlower_outlier(trucks['Freq'])
out_freq=trucks.loc[(trucks['Freq']>ul1)|(trucks['Freq']<ll1),]
print(out_freq)

##Outlier treatement
trucks['Freq']=treat_outlier_ul_ll_winsor(trucks['Freq'])
trucks.boxplot()

##target variable proprotion
trucks['collision'].value_counts().plot(kind='bar')




##Splitting into train ,test 
all_labels=trucks['collision']
all_ind_ds=trucks.drop(["collision"],axis=1)
...
##Scaling 
sobj=StandardScaler()
scaled_trainset=sobj.fit_transform(trainset)
scaled_testset=sobj.transform(testset)
trucks_trainset=pd.DataFrame(scaled_trainset,columns=trainset.columns)
trucks_testset=pd.DataFrame(scaled_testset,columns=testset.columns)
##Boxplot after scaling to see all trucks 
trucks_trainset.boxplot()
plt.xticks(rotation=90);

lda_classifier_obj = LinearDiscriminantAnalysis()
lda_mod=lda_classifier_obj.fit(trucks_trainset,train_labels)
lda_mod=lda_classifier_obj.fit(all_ind_ds,all_labels)

pred=lda_mod.predict(all_ind_ds)
print(accuracy_score(all_labels,pred))
print(f1_score(all_labels,pred))
# Training Data Class Prediction with a cut-off value of 0.5
pred_class_train = lda_mod.predict(trucks_trainset)

# Test Data Class Prediction with a cut-off value of 0.5
pred_class_test = lda_mod.predict(trucks_testset)
...
test_acc=round(accuracy_score(test_labels, pred_class_test),4)*100

print(test_acc)


print('Area Under the Curve is',round(roc_auc_score(test_labels,lda_mod.predict_proba(trucks_testset)[:,1]),2)*100,'%')
