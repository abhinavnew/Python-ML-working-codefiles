# -*- coding: utf-8 -*-
"""
Created on Fri Jul 16 16:36:29 2021
...
spark_orig=pd.read_csv(r'E:\AbhinavB\Kaggle\Python related\DSBA related\Competions\TimeSeries_Project\Sparkling.csv')
print(spark_orig.dtypes)
print(spark_orig.shape)
print(spark_orig.head())

spark=spark_orig.copy()
##creating a timestamp (should know first date )
time_stamp=pd.date_range(start='1980-01-01',periods=len(spark),freq='M')
##date = pd.date_range(start='1/1/1959', periods=len(df), freq='D')

print(time_stamp)
spark['Time_Stamp']=time_stamp
spark.set_index(keys='Time_Stamp',inplace=True)

spark.head()

spark.drop(spark.columns[spark.columns.str.contains('unnamed',case=False)],axis=1,inplace=True)
...
spark_add_decompose = seasonal_decompose(spark, model = "additive")
spark_add_decompose.plot()


trend = spark_add_decompose.trend
seasonality = spark_add_decompose.seasonal
residual = spark_add_decompose.resid
...
spark_mul_decompose = seasonal_decompose(spark, model = "multiplicative")
spark_mul_decompose.plot()


trend_m = spark_mul_decompose.trend
seasonality_m = spark_mul_decompose.seasonal
residual_m = spark_mul_decompose.resid
...
spark_log['Sparkling']=np.log(spark)
spark_log['Sparkling']
spark_log.plot()
plt.subplot(2,1,1)
...
####Train Test split 
spark.index.year.unique()

trainset = spark[spark.index<='1990'] 
testset = spark[spark.index>'1990']
...
NaiveModel_test['naive'] = np.asarray(trainset['Sparkling'])[len(np.asarray(trainset['Sparkling']))-1]
NaiveModel_test['naive'].head()
##plot naive model
plt.plot(NaiveModel_train['Sparkling'], label='Train')
plt.plot(testset['Sparkling'], label='Test')
...
##metric -RMSE 
rmse_model2_test = metrics.mean_squared_error(testset['Sparkling'],NaiveModel_test['naive'],squared=False)
print("For Naive Model forecast on the Test Data,  RMSE is %3.3f" %(rmse_model2_test))
resultsDf = pd.DataFrame({'Test RMSE': [em.rmse(testset.values,NaiveModel_test['naive'])[0]]},index=['Naive'])
print(resultsDf)
...
SimpleAverage_test['mean_forecast'] = trainset['Sparkling'].mean()
...
##metric -RMSE 
rmse_model3_test = metrics.mean_squared_error(testset['Sparkling'],SimpleAverage_test['mean_forecast'],squared=False)
...
rmse_model4_test_2 = metrics.mean_squared_error(testset['Sparkling'],trailing_MovingAverage_test['Trailing_2'],squared=False)
print("For 2 point Moving Average Model forecast on the Training Data,  RMSE is %3.3f" %(rmse_model4_test_2))

## Test Data - RMSE --> 4 point Trailing MA
rmse_model4_test_4 = metrics.mean_squared_error(testset['Sparkling'],trailing_MovingAverage_test['Trailing_4'],squared=False)
print("For 4 point Moving Average Model forecast on the Training Data,  RMSE is %3.3f" %(rmse_model4_test_4))

## Test Data - RMSE --> 6 point Trailing MA

rmse_model4_test_6 = metrics.mean_squared_error(testset['Sparkling'],trailing_MovingAverage_test['Trailing_6'],squared=False)
print("For 6 point Moving Average Model forecast on the Training Data,  RMSE is %3.3f" %(rmse_model4_test_6))

## Test Data - RMSE --> 9 point Trailing MA

rmse_model4_test_9 = metrics.mean_squared_error(testset['Sparkling'],trailing_MovingAverage_test['Trailing_9'],squared=False)
...
print("predicted vector of values\n",SES_predict)
...
resultsDf_temp = pd.DataFrame({'Test RMSE': [em.rmse(testset.values,SES_predict.values)[0]]},index=['SES'])
...
print("Double Exp Smoothing predictions=\n",DES_predict)
...
resultsDf_temp = pd.DataFrame({'Test RMSE': [mean_squared_error(testset.values,DES_predict.values,squared=False)]}
                           ,index=['DES'])
...
plt.plot(DES_predict, label='Double Exponential Smoothing predictions on Test Set')
plt.plot(TES_predict, label='HW-Additive predictions on Test Set')
...
resultsDf_temp = pd.DataFrame({'Test RMSE': [mean_squared_error(testset.values,TES_predict.values,squared=False)]}
                           ,index=['Additive :TES or HW'])
...
resultsDf_temp = pd.DataFrame({'Test RMSE': [mean_squared_error(testset.values,TES_predict_am.values,squared=False)]}
                           ,index=['Multipicative :TES or HW'])
...
dftest = adfuller(spark,regression='ct')
print('DF test statistic is %3.3f' %dftest[0])
print('DF test p-value is' ,dftest[1])
print('Number of lags used' ,dftest[2])

##p-val (0.70559)! <0.05 hence failure to reject NULL hypothesis -Series is NON STATIONARY

##Let us take one level of differencing to see whether the series becomes stationary.

dftest_1 = adfuller(spark.diff().dropna(),regression='ct')
print('DF test statistic is %3.3f' %dftest_1[0])
print('DF test p-value is' ,dftest_1[1])
print('Number of lags used' ,dftest_1[2])

##Since p val =0 which <0.05 ,hence reject null hyp ,Timeseries is now stationary 
...
for param in pdq:# running a loop within the pdq parameters defined by itertools
    ARIMA_model = ARIMA(trainset['Sparkling'].values,order=param).fit()#fitting the ARIMA model
...
auto_ARIMA_best = ARIMA(trainset, order=(2,1,2))
...
rmse_arima_auto = mean_squared_error(testset['Sparkling'],predicted_auto_ARIMA_best,squared=False)
...
#The Auto-Regressive parameter in an ARIMA model is 'p' which comes from the significant lag before which the PACF plot cuts-off at 3
...
manual_ARIMA = ARIMA(trainset['Sparkling'].astype('float64'), order=(3,1,2),freq='M')
...
for param in pdq:
    for param_seasonal in model_pdq:
        SARIMA_model = sm.tsa.statespace.SARIMAX(trainset['Sparkling'].values,
                                            order=param,
...
auto_SARIMA_best = sm.tsa.statespace.SARIMAX(trainset['Sparkling'].values,
                                order=(1, 1, 2),
...
rmse_auto_sarima_best = mean_squared_error(testset['Sparkling'],predicted_auto_SARIMA_best.predicted_mean,squared=False)
print(rmse_auto_sarima_best)
...
test_stationarity((trainset['Sparkling'].diff(12).dropna()).diff(1).dropna())

##pval <0.05 hence this TS is stationary 

##step 3 -Checking the ACF and the PACF plots for the new modified Time Series.

plot_acf((spark['Sparkling'].diff(6).dropna()).diff(1).dropna(),lags=30)
plot_pacf((spark['Sparkling'].diff(6).dropna()).diff(1).dropna(),lags=30)

##Seasonal AR order -PACF -tapers to zero after 2 lags --Each lag here is multiple of F ie 12 ;P=2
...
manual_SARIMA = sm.tsa.statespace.SARIMAX(trainset['Sparkling'].values,
                                order=(3, 1, 2),
...
rmse_manual_sarima = mean_squared_error(testset['Sparkling'],predicted_manual_sarima.predicted_mean,squared=False)
print(rmse_auto_sarima_best)


#### Result collation 
index=['Naive Approach','Simple Average Menthod','Moving Average-2Point','SES','DES','HW-A','HW-M','ARIMA-Auto(2,1,2)','ARIMA-Manual(3,1,2)','SARIMA-Auto(1,1,2)(2,0,2,12)','SARIMA-Manual(3,1,2)(2,1,2,12)']
data = pd.DataFrame({
...
plt.plot(pred_manual_sarima_date['mean'] , label='SARIMA-manual(3,1,2)(2,1,2,12)')
plt.legend(loc='best')
plt.grid()
...
##HW -Multiplicative give best results 

opt_hw_am = ExponentialSmoothing(spark,trend='add',seasonal='multiplicative',initialization_method='estimated')
# Fitting the model
opt_hw_am= opt_hw_am.fit()

print('')
print('==Holt Winters model Exponential Smoothing Estimated Parameters ==')
print('')
print(opt_hw_am.params)

##Find RMSE on whole of data from the optimum model
rmse2 = mean_squared_error(spark['Sparkling'],opt_hw_am.fittedvalues,squared=False)
print('RMSE of the Full fitted HW-(M) Model on complete data set',rmse2)

index=['Naive Approach','Simple Average Menthod','Moving Average-2Point','SES','DES','HW-A','HW-M','ARIMA-Auto(2,1,2)','ARIMA-Manual(3,1,2)','SARIMA-Auto(1,1,2)(2,0,2,12)','SARIMA-Manual(3,1,2)(2,1,2,12)','Optimum HW-M model on Full data']
data2 = pd.DataFrame({
...
# Forecasting using this model for the duration of the test set
opt_tes_predict =  opt_hw_am.forecast(steps=12)
print(opt_tes_predict)


####Confidence Interval for optimum model
#In the below code, we have calculated the upper and lower confidence bands at 95% confidence level
#Here we are taking the multiplier to be 1.96 as we want to plot with respect to a 95% confidence intervals.
opt_mod_ci = pd.DataFrame({'lower_CI':opt_tes_predict - 1.96*np.std(opt_hw_am.resid,ddof=1),
                          'prediction':opt_tes_predict,
                          'upper_ci': opt_tes_predict+ 1.96*np.std(opt_hw_am.resid,ddof=1)})
opt_mod_ci
