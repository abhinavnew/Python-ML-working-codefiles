...
# encobj_desig1=OrdinalEncoder(categories=[['Tier-1' ,'Tier-2' ,'Tier-3']])
# trainonly_emp['College_Tier']=encobj_desig1.fit_transform(trainonly_emp[["College_Tier"]])  

trainonly_emp['College_Tier']=trainonly_emp.College_Tier.map({'Tier-1':2,'Tier-2':1,'Tier-3':0})
unktestset['College_Tier']=unktestset.College_Tier.map({'Tier-1':2,'Tier-2':1,'Tier-3':0})

##Features 
#Binning the Age 

bins = [15,20,30,45,63]
label =['Age 20 and below ','Age 20-30','Age 31-45','Age 45 and above']
trainonly_emp['Age_Bracket'] = pd.cut(trainonly_emp['Age'],bins ,right=True,labels=label)
trainonly_emp['Age_Bracket']=trainonly_emp.Age_Bracket.map({'Age 20 and below ':0,'Age 20-30':1,'Age 31-45':2,'Age 45 and above':3})
trainonly_emp['Age_Bracket']=trainonly_emp['Age_Bracket'].astype(int)
print(trainonly_emp.head(20))
...
cols_to_use=['College_Tier','EnvironmentSatisfaction1','EnvironmentSatisfaction2','JobSatisfaction1','JobSatisfaction2','JobInvolvement1','JobInvolvement2','PerformanceRating1','PerformanceRating2','YearsSinceLastPromotion','NumCompaniesWorked']
...
imputer=SklearnTransformerWrapper(transformer =KNNImputer(
                                 weights='distance',metric='nan_euclidean'),
                                  variables=cols_to_use
                                  )



imputer.fit(knntrain,knnlabels)

print("Running KNN Imputer on Trainset now ....")
trainonly_emp_t=imputer.transform(knntrain)
trainonly_emp_label=pd.Series(knnlabels.astype(int),name="Employeeturnover")
trainonly_emp_imp=pd.concat([trainonly_emp_label,trainonly_emp_t],axis=1)

print("Running KNN Imputer on Unknown testset now ....")
unktestset_imp=imputer.transform(unktestset)

print("null values in full trainset ",trainonly_emp_imp.isnull().sum())
print("null values in full unknown testset ",unktestset_imp.isnull().sum())

####Merging train and unknown test set 
trainonly_emp_imp['Ind']="train"

unktestset_imp['Employeeturnover']=np.nan
unktestset_imp['Ind']="unknown_test"

frames=[trainonly_emp_imp,unktestset_imp]
...
print("Not dropping the id field ",emp.shape)
emp['ID']=np.log(emp['ID'])
print(emp['ID'])
emp['ID'].plot()
print("log transform the id field ",emp.shape)
...
print("Full train shape after demerging ",emp_train.shape)
print("Full unknown test shape after demerging ",test_unknown.shape)
...
param_grid = {
    ...
    'early_stopping_rounds':[10],
    'subsample':[0.8],
    #'ColSamplebyTree':[0.6,0.9],
    #'Lambda':[0.5],
    #'alpha':[0.2]
}


xgb_mod=GridSearchCV(estimator=xgbobj,param_grid=param_grid,cv=5,verbose=20,scoring='f1')
xgb_mod.fit(trainset,train_labels,eval_metric="error")
...
####Training best estimator on entire training data 
xgb_mod2=xgb_mod_bst.fit(all_ind_ds,all_labels)
pred2=xgb_mod2.predict(all_ind_ds)
pred3=xgb_mod2.predict(testset)
print(pred2.size)
print("f1 accuracy on best model BUT fitted to all training data =",f1_score(pred2,all_labels))
print("f1 accuracy (VALIDATE SET ONLY) on best model BUT fitted to all training data =",f1_score(pred3,test_labels))
acc_full_xgb=f1_score(pred2,all_labels)

####SMOTE on entire training data 
from imblearn.over_sampling import SMOTE 

oversample=SMOTE()

smote_train,smote_labels=oversample.fit_resample(all_ind_ds,all_labels)
print(all_ind_ds.shape)
print(len(all_labels))
print(smote_train.shape)
print(len(smote_labels))

xgb_mod3=xgb_mod_bst.fit(smote_train,smote_labels)
pred_smote=xgb_mod3.predict(all_ind_ds)
print(pred_smote.size)
print("f1 accuracy on best model BUT fitted to all training data =",f1_score(pred_smote,all_labels))

##AUC score SMOTE FULL DATASET 
probs9 = xgb_mod3.predict_proba(all_ind_ds)
# keep probabilities for the positive outcome only
probs9= probs9[:, 1]
##AUC 
auc9 = roc_auc_score(all_labels, probs9)
...
probs7 = xgb_mod2.predict_proba(all_ind_ds)
# keep probabilities for the positive outcome only
probs7= probs7[:, 1]
##AUC 
auc7 = roc_auc_score(all_labels, probs7)
...
for j in np.arange(0.1,1,0.05):
    ...
    for i in range(0,len(all_labels)):#defining a loop for the length of the train data
        if np.array(probs9)[i] > custom_prob:#issuing a condition for our probability values to be 
            #greater than the custom cutoff value
            a=1#if the probability values are greater than the custom cutoff then the value should be 1
        else:
            a=0#if the probability values are less than the custom cutoff then the value should be 0
        custom_cutoff_data.append(a)#adding either 1 or 0 based on the condition to the end of the list defined by us
    print(round(custom_prob,3),round(metrics.f1_score(all_labels,custom_cutoff_data),4))
##0.4--->F1=0.9051 highest

data_pred_custom_cutoff_validate=[]
for i in range(0,len(probs7)):
    if np.array(probs7)[i]>0.55:
        a=1
    else:
        a=0
...
acc_fullset_optthresh=f1_score(all_labels,data_pred_custom_cutoff_validate)
...
dataf = pd.DataFrame({
        ...
        'XBGFull':[acc_full_xgb,auc7],
        'XGBFullOptThresh':[acc_fullset_optthresh,auc7]},index=index)
round(dataf,5)
...
pred_unk_smote=xgb_mod3.predict(test_unknown)
pred_unk_smote=pd.Series(pred_unk_smote.astype(int),name="Employeeturnover")
result_smote=pd.concat([id,pred_unk_smote],axis=1)

probs10 = xgb_mod3.predict_proba(test_unknown)
# keep probabilities for the positive outcome only
probs10= probs10[:, 1]

data_pred_custom_cutoff_unk=[]
for i in range(0,len(probs10)):
    if np.array(probs10)[i]>0.55:
        a=1
    else:
        a=0
...
pred_unk_xgboptthresh=data_pred_custom_cutoff_unk
...
pred_unk_xgboptthresh=pd.Series(pred_unk_xgboptthresh,name="Employeeturnover")
...
result_xgbfulloptthresh=pd.concat([id,pred_unk_xgboptthresh],axis=1)
...
myfile_name2='xgbfullOptthresh_'+snapshotdate+'.csv'
print(myfile_name)
...
result_xgbfulloptthresh.to_csv(p+myfile_name2,index=False)
#result_smote.to_csv(p+myfile_name,index=False)
