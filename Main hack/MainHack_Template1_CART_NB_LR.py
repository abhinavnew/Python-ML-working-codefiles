# -*- coding: utf-8 -*-
"""
Created on Sun Jul 25 19:56:19 2021
...
nb_model2=GridSearchCV(estimator=nb_obj,param_grid=param_grid_nb,cv=10,verbose=20,scoring=f1_score)
...
print("training time of Naive Bayes model= ",train_time)
...
##Accuracy score 
acc_score_train_bst_nb=f1_score(train_labels,pred_train_bst_nb)
print("F1 score of Tuned nb model on TRAIN set=",acc_score_train_bst_nb)
acc_score_test_bst_nb=f1_score(test_labels,pred_test_bst_nb)
print("F1 score of Tuned nb model on TEST set=",acc_score_test_bst_nb)
##AUC scores
##Train
...
# calculate AUC
auc_train_nb = roc_auc_score(train_labels, probs_train_nb)
print('AUC on Train set for NB basic model: %.3f' % auc_train_nb)
##test
probs_test_nb = nb_model_bst.predict_proba(testset)
# keep probabilities for the positive outcome only
probs_test_nb = probs_test_nb[:, 1]
# calculate AUC
auc_test_nb = roc_auc_score(test_labels, probs_test_nb)
print('AUC on Train set for NB basic model: %.3f' % auc_test_nb)
# calculate roc curve 
#train
train_fpr_nb, train_tpr_nb, train_thresholds_nb = roc_curve(train_labels, probs_train_nb)
plt.plot([0, 1], [0, 1], linestyle='--')
# plot the roc curve for the model
plt.plot(train_fpr_nb, train_tpr_nb)
#test
test_fpr_nb, test_tpr_nb, test_thresholds_nb = roc_curve(test_labels, probs_test_nb)
plt.plot([0, 1], [0, 1], linestyle='--')
# plot the roc curve for the model
...
log_acc_train=f1_score(train_labels,pred_train_log)
print("F1 score on trainset =",f1_score(train_labels,pred_train_log))
pred_test_log=model_logis.predict(testset)
print(confusion_matrix(test_labels,pred_test_log))
log_acc_test=f1_score(test_labels,pred_test_log)
print(log_acc_test)
print("F1 Score on Validateset=",f1_score(test_labels,pred_test_log)) 
##Auc train
...
##{'criterion': 'entropy', 'max_depth': 10, 'max_features': 25, 
...
rf_acc_score_train=f1_score(train_labels,pred_train_gs2)
print("Trainset f1 score for RF model=",rf_acc_score_train)
print(f1_score(train_labels,pred_train_gs2))
##Testset
pred_test_gs2=rfc_mod_bst2.predict(testset)
print(confusion_matrix(test_labels,pred_test_gs2))
rf_acc_score_test=f1_score(test_labels,pred_test_gs2)
print("validate Set F1 Score for RF model=",rf_acc_score_test)
print(f1_score(test_labels,pred_test_gs2))
...
cart_train_acc=round(f1_score(train_labels, pred_cart_train1),4)*100
print("F1 Score on trainset CART MODEL is",cart_train_acc)
print(f1_score(train_labels, pred_cart_train1))
...
cart_test_acc=round(f1_score(test_labels, pred_cart_test1),4)*100
print("F1 Score on validate set CART MODEL is",cart_test_acc)
print(f1_score(test_labels, pred_cart_test1))
...
nn_mod1 = GridSearchCV(estimator = nn_cl, param_grid = param_grid3, cv = 6,verbose=20)
...
acc_nn_train=f1_score(train_labelsnn,pred_train_nn)
print(acc_nn_train)
print("F1 Score on the Train set  =",f1_score(train_labelsnn,pred_train_nn))
...
print(acc_nn_test)
print("F1 Score on the TEST set  =",f1_score(test_labelsnn,pred_test_nn))
...
fpr4, tpr4, thresholds4 = roc_curve(train_labelsnn, probs4)
plt.plot([0, 1], [0, 1], linestyle='--')
# plot the roc curve for the model
...
probs3= probs3[:, 1]
# calculate AUC
auc3 = roc_auc_score(train_labelsnn, probs4)
print('AUC of NN model on TEST SET  %.3f ' % auc3)
##Calc ROC curve and threshold 
fpr3, tpr3, thresholds3 = roc_curve(train_labelsnn, probs4)
plt.plot([0, 1], [0, 1], linestyle='--')
# plot the roc curve for the model
...
pred_unk_cart=pd.Series(pred_unk_cart.astype(int),name="Attrition")
...
pred_unk_nn=pd.Series(pred_unk_nn,name="Attrition")
##combine=np.vstack((id,pred_unk_cart)).T
#combine=np.vstack((id,pred_unk_cart)).T
result_cart=pd.concat([id,pred_unk_cart],axis=1)
result_nn=pd.concat([id,pred_unk_nn],axis=1)
